...
|---|------|------|-------------|
| F1 | ✅ **2026-07-02** AutoStyler slot-fill — forced back-budding | Cross-system | Root cause: tree AT maxBranchNodes → stimulated buds silently discarded every year. Fix: styler-directed buds force-break (≤3/spring, energy-gated), bypassing cap+roll. **Verify in-game:** `[Bud] … forced slot-fill=N`, candidates >6, Empty falling |
